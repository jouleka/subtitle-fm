...
function ev(
  over: Partial<{ key: string; shiftKey: boolean; metaKey: boolean; isComposing: boolean; keyCode: number }> = {},
) {
...
test("Tab navigates next, Shift+Tab navigates previous", () => {
  ...
  expect(classifyCueKeydown(ev({ key: "Tab", shiftKey: true }), false)).toEqual({ type: "nav", direction: "prev" });
});
test("Cmd+ArrowUp/Down move the cue", () => {
  expect(classifyCueKeydown(ev({ key: "ArrowUp", metaKey: true }), false)).toEqual({ type: "move", direction: "up" });
  expect(classifyCueKeydown(ev({ key: "ArrowDown", metaKey: true }), false)).toEqual({ type: "move", direction: "down" });
});
...
test("isComposing on the event is none", () => {
  expect(classifyCueKeydown(ev({ key: "Enter", isComposing: true }), false)).toEqual({ type: "none" });
});
