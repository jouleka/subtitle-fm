...
const FAKE_USER = {
  ...
  handle: 'test-user-sfm28',
  email: 'test-sfm28@example.com',
};
const FAKE_SESSION = {
  id: '44444444-4444-4444-4444-444444444444',
  userId: FAKE_USER.id,
  token: 'test-token-sfm28',
...
beforeAll(async () => {
  ...
  await db
    .insert(schema.shows)
    .values({ id: TEST_SHOW_ID, title: 'Test Show SFM-28', slug: 'test-show-sfm-28-glossary' })
    .onConflictDoNothing();
});
afterAll(async () => {
  await cleanup();
...
describe('glossary CRUD', () => {
  ...
  test("POST creates a term (201) and GET lists it (intent: panel reads a show's terms)", async () => {
    authed();
    const created = await jsonReq('POST', base, {
      sourceText: '先輩',
      targetText: 'senpai',
      kind: 'honorific',
    });
  ...
  test('PATCH a nonexistent term returns 404', async () => {
    authed();
    const res = await jsonReq('PATCH', `${base}/00000000-0000-0000-0000-000000000000`, {
      targetText: 'x',
    });
  ...
  test('GET an unknown show returns an empty list, not 404 (intent: glossary list is lean — no existence check, unlike cues)', async () => {
    const res = await app.request('/shows/no-such-show-sfm28/glossary');
    expect(res.status).toBe(200);
    expect(((await res.json()) as { glossaryTerms: unknown[] }).glossaryTerms).toHaveLength(0);
  });
});
