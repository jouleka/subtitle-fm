...
export const glossary = new Hono<{ Variables: AuthVariables }>()
  ...
  .post('/', requireSession, zValidator('json', CreateGlossaryTerm), async (c) => {
    ...
    const dupe = await db
      ...
      .from(schema.glossaryTerms)
      .where(and(eq(schema.glossaryTerms.showId, showId), eq(schema.glossaryTerms.sourceText, input.sourceText)))
      .limit(1);
    if (dupe.length) return c.json({ error: 'duplicate_source' }, 409);
    const [row] = await db
      .insert(schema.glossaryTerms)
      .values({ showId, sourceText: input.sourceText, targetText: input.targetText, kind: input.kind, notes: input.notes ?? null })
      .returning();
