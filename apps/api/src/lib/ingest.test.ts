...
describe("ensureShow", () => {
  test("creates a missing show", async () => {
    ...
    const [row] = await db.select({ id: schema.shows.id }).from(schema.shows).where(eq(schema.shows.id, "cat-a")).limit(1);
  ...
  test("returns exists for an existing id and does NOT update it (intent: v1 is create-if-missing, never overwrite)", async () => {
    ...
    const [row] = await db.select({ title: schema.shows.title }).from(schema.shows).where(eq(schema.shows.id, "cat-a")).limit(1);
  ...
  test("throws ShowConflictError on a slug collision with a new id (intent: a clear operator error, not a raw 23505)", async () => {
    await db.insert(schema.shows).values({ id: "cat-slug-owner", title: "Owner", slug: "shared-slug" });
    let err: unknown;
    try {
...
describe("importCatalog", () => {
  test("creates shows + episodes and enqueues one job per created episode", async () => {
    const res = await importCatalog([{ id: "cat-a", title: "A", slug: "cat-a", episodes: epList(2) }]);
    ...
    const rows = await db.select({ id: schema.episodes.id }).from(schema.episodes).where(eq(schema.episodes.showId, "cat-a"));
  ...
  test("isolates a failing show: A succeeds, B fails (slug collision), C still succeeds (intent: one bad show never aborts the batch)", async () => {
    await db.insert(schema.shows).values({ id: "cat-slug-owner", title: "Owner", slug: "taken-slug" });
