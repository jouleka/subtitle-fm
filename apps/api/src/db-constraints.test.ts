...
const IDS = [...EXTERNAL_ID_COLS.flatMap((c) => [`sfm64-${c}-a`, `sfm64-${c}-b`]), ...NULL_IDS];
...
describe('episodes season-aware uniqueness (SFM-66)', () => {
  ...
  // tests encode that (show_id, season_id, number) is a hard DB invariant.
  ...
  beforeAll(async () => {
    ...
    const inserted = await db
      .insert(schema.seasons)
      .values([
        { showId: SHOW, number: 1 },
        { showId: SHOW, number: 2 },
      ])
      .returning({ id: schema.seasons.id, number: schema.seasons.number });
    season1Id = inserted.find((s) => s.number === 1)!.id;
    season2Id = inserted.find((s) => s.number === 2)!.id;
  });
  afterAll(async () => {
  ...
  test('rejects a duplicate within one season', async () => {
    await db.insert(schema.episodes).values({ showId: SHOW, seasonId: season1Id, number: 1 });
    let threw = false;
    try {
      await db.insert(schema.episodes).values({ showId: SHOW, seasonId: season1Id, number: 1 });
    } catch {
  ...
  test('allows the same episode number in different seasons', async () => {
    const [ep] = await db
      .insert(schema.episodes)
      .values({ showId: SHOW, seasonId: season2Id, number: 1 })
      .returning({ id: schema.episodes.id });
    expect(ep!.id).toBeTruthy();
  });

  test('treats two null seasons as the same identity', async () => {
    await db.insert(schema.episodes).values({ showId: SHOW, number: 99 });
    let threw = false;
    try {
      await db.insert(schema.episodes).values({ showId: SHOW, number: 99 });
    } catch {
